...
/**
 * Fixed & simplified: show a numeric wrapper based on a radio group.
 * We consider the wrapper "on" if the checked radio's value !== "off".
 */
export function setupThresholdToggle(groupId, radioName, wrapperId, defaultValue) {
  const _group = document.getElementById(groupId); // kept for API parity; not required
  const wrapper = document.getElementById(wrapperId);
  ...
  const apply = () => {
    ...
    const on = !!checked && String(checked.value).toLowerCase() !== 'off';
    wrapper.classList.toggle('hidden', !on);
    if (on) {
      const input = wrapper.querySelector('input[type="number"]');
      if (input && (input.value === '' || input.value == null)) input.value = defaultValue;
...
export function wireSplitDependency({ mainBtnId, extraBtnId, mainToggleId, extraToggleId }) {
